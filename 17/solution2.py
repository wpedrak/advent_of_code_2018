...
def have_side_sand(board, point):
    x, y = point
    sides = [
        board[y][x-1],
        board[y][x+1]
    ...
    return EMPTY in sides

def dry_tick(board, min_x, max_x):
    changes = 0
    for y, row in enumerate(board):
        for x in range(min_x-1, max_x+2):
            point = (x, y)
            if not is_water(board, point):
                continue
            if not have_side_sand(board, point):
                continue
...
def dry_water(board, min_x, max_x):
    changes = 1
    cnt = 0
    while changes:
        changes = dry_tick(board, min_x, max_x)
        print(cnt, changes)
        cnt += 1
